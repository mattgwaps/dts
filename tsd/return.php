...
if (isset($_POST['submit'])) {
    ...
    if ($penrodepartment === "TSD") {
        ...
        header("Location:tsd/tsdprocesstable.php?denrtransactionno=$denrtransactionno");
        exit();
    }
//duplicate checker
    $query = mysqli_query($conn,$sqlduplicatechecker);
    if(mysqli_num_rows($query) >0){
        $duplicate = "tsdreturnform.php?d&denrtransactionno=" . urlencode($denrtransactionno);
        header("Location: " . $duplicate);
        
        exit; 
    }else{
    // Prepare insert statement
    $sql = "INSERT INTO $table (denrtransactionno, denrname, denrdaterecv, denrformtime, denrsubject, denrdocumentsource, denrdocumentstatus, denrdatetransmitted, denrofficeorigin, returnpurpose, returndate, returntime, penropriority, penroprocessor, penrodateprocessed, penroformtime, penroaction, penrodepartment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    
    // Bind parameters
    mysqli_stmt_bind_param($stmt, "ssssssssssssssssss", $denrtransactionno, $denrname, $denrdaterecv, $denrformtime, $denrsubject, $denrdocumentsource, $denrdocumentstatus, $denrdatetransmitted, $denrofficeorigin, $returnpurpose, $returndate, $returntime, $penropriority, $penroprocessor, $penrodateprocessed, $penrotime, $penroaction, $penrodepartment);
