...
<head>
  ...
  <title>127.0.0.1/Processtable/<?php echo $_SESSION['username']; ?></title>
<link rel="stylesheet" href="/DENR/css/division/processtable.css">
   <!-- Bootstrap Bundle with Popper -->
   <script defer src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"></script>
   <script defer src="/DENR/js/tsdprocesstable.js"></script>
...
<div style=""><input type="text" name="inputSearch"> <input type="submit" name="search" value="SEARCH"></div>
  ...
  <?php
          ...
          if($search === ""){
            ...
            echo "</tr></tbody>";
             ...
             $sql = "SELECT * FROM tsd_table WHERE penrodepartment = 'TSD' AND denrtransactionno LIKE ? OR denrofficeorigin LIKE ? OR denrdocumentstatus LIKE ? OR denrsubject LIKE ? AND penrodepartment = 'TSD' ORDER BY denrtransactionno DESC";
             ...
             if(mysqli_num_rows($result) > 0) {
              while($rows = mysqli_fetch_assoc($result)) {
                ...
                echo "<td class='border border-white border-2' style='text-align:center;'><div><a href='tsdreturnform.php?denrtransactionno=" . $rows['denrtransactionno'] . "'>Return</a></div></td>";   
                echo "</tr></tbody>";
            }
        } else {
            echo "<tbody><tr>";
  ...
  //Code for Displaying Records
    ...
    $sql = "SELECT * FROM tsd_table WHERE penrodepartment = 'TSD' ORDER BY id DESC";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
