...
<head>
  ...
  <title>127.0.0.1/Documents/<?php echo $_SESSION['username']; ?></title>
   <link rel="stylesheet" href="/DENR/css/division/table.css">
   <!-- Bootstrap Bundle with Popper -->
   <script defer src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/js/bootstrap.bundle.min.js"></script>
   <script defer src="/DENR/js/tsdtable.js"></script>
...
<div style=""><input type="text" name="inputSearch"> <input type="submit" name="search" value="SEARCH"></div>
  ...
  <?php
          ...
          if($search === ""){
            ...
            $sql = "SELECT * FROM document_table WHERE (denrtransactionno LIKE ? OR denrofficeorigin LIKE ? OR denrdocumentstatus LIKE ? OR denrsubject LIKE ?) AND penrodepartment = 'TSD' ORDER BY denrtransactionno DESC";
  ...
  //Code for Displaying Records
    ...
    $sql = "SELECT * FROM document_table where penrodepartment = 'TSD' ORDER BY id DESC";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    if(mysqli_num_rows($result) > 0) {
      while($rows = mysqli_fetch_assoc($result)) {
        ...
        echo "<tbody><tr>";
        echo "<td colspan='13' text-align:center;>EMPTY</td>";
        echo "</tr></tbody>";
    }
  } 
