...
<body background="/denr/img/ur.jpg">
  ...
  <div class="container mt-5 p-5 bg-dark text-light" style="overflow:auto; height:50vh;">
...
<div style=""><input type="text" name="inputSearch"> <input type="submit" name="search" value="SEARCH"></div>
  ...
  <?php
          ...
          if($search === ""){
            ...
            $sql = "SELECT * FROM document_table WHERE (denrtransactionno LIKE ? OR denrofficeorigin LIKE ? OR denrdocumentstatus LIKE ? OR denrsubject LIKE ?) AND penrodepartment = 'MSD' ORDER BY denrtransactionno DESC";
  ...
  //Code for Displaying Records
    ...
    $sql = "SELECT * FROM document_table where penrodepartment = 'MSD' ORDER BY id DESC";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    if(mysqli_num_rows($result) > 0) {
      while($rows = mysqli_fetch_assoc($result)) {
        ...
        echo "<tbody><tr>";
        echo "<td colspan='13' style='text-align:center;'>EMPTY</td>";
        echo "</tr></tbody>";
    }
  } 
