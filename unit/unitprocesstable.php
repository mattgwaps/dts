...
<head>
  ...
  <link rel="stylesheet" href="/DENR/css/unit/unitprocesstable.css">
    ...
    <img src="/denr/img/logo.ico" alt="logo" style="width:3rem"> <a class="navbar-brand">PENRO LEYTE</a>
      ...
      <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <li class="nav-item">
          ...
          <li class="nav-item">
            <a class="nav-link" href="unithome.php">BACK</a>
          </li>
        </ul>
      </div>
    </div>
...
<table id="example" class="display" style="width:100%" style="overflow:auto;">
<form action="" method="POST">
<div style="display:flex; justify-content:space-between; margin-bottom:10px;">
<div style="background:#213438; font-weight:bold; padding:5px;"><span>Process Table</span></div>
<div style=""><input type="text" name="inputSearch"> <input type="submit" name="search" value="SEARCH"></div>
</form>
</div>
        <thead>
            <tr>
  ...
  <?php
          ...
          if($search === ""){
            echo "<tbody><tr>";
            echo "<td colspan='7' style= 'color:#FF6969; font-weight:bold; width:100%; text-align:center;'>Error: Search should not be empty or contain whitespace.</td>";
            echo "</tr></tbody>";
          }else{
             $sql = "SELECT * FROM unit_table WHERE deptworkgroup = '$workgroup' AND denrtransactionno LIKE ? OR denrofficeorigin LIKE ? OR denrdocumentstatus LIKE ? OR denrsubject LIKE ? AND penrodepartment = 'TSD' ORDER BY denrtransactionno DESC";
             ...
             if(mysqli_num_rows($result) > 0) {
              while($rows = mysqli_fetch_assoc($result)) {
                ...
                }else if ($rows['denrdocumentstatus'] == "Transmitted") {
  ...
  //Code for Displaying Records
    ...
    $sql = "SELECT * FROM unit_table WHERE deptworkgroup = '$workgroup' ORDER BY denrtransactionno DESC";
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
function updateRow(denrtransactionno){
    location.href = "unitform.php?denrtransactionno=" + denrtransactionno;
    }
    //-------------------------------
